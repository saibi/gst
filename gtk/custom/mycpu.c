...
static void my_cpu_class_init(MyCpuClass *klass)
{
	...
	// install property
	pspec = g_param_spec_double("percent", "Percent", "What CPU load should be displayed", 0, 1, 0, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS);
...
static void my_cpu_realize(GtkWidget *widget)
{
	...
	widget->style = gtk_style_attach(gtk_widget_get_style(widget), priv->window);
