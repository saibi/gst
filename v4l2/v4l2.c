...
int use_parse_launch(CustomData *data)
{
	...
	data->pipeline = gst_parse_launch("rkisp device=/dev/video0 io-mode=1 analyzer=1 enable-3a=1 ! video/x-raw,format=NV12,width=640,height=480,framerate=30/1 ! videoconvert ! kmssink name=videosink render-rectangle='<100,100,320,240>", NULL);
